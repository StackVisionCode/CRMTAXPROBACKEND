...
public class ConfirmAccountHandler
    : IRequestHandler<AccountConfirmCommands, ApiResponse<Unit>>
{
  ...
  public ConfirmAccountHandler(ApplicationDbContext db,
  ...
  public async Task<ApiResponse<Unit>> Handle(AccountConfirmCommands c, CancellationToken ct)
  {
    ...
    if (user is null) return new(false, "Cuenta no encontrada");

    if (user.Confirm is true) return new(false, "La cuenta ya está confirmada");

    if (user.ConfirmToken != c.Token) return new(false, "Token inválido");
    ...
    _eventBus.Publish(new AccountConfirmedEvent(
        Guid.NewGuid(), DateTime.UtcNow,
        user.Id, user.Email, display));
