...
public class GetAllSessionsHandler : IRequestHandler<GetAllSessionsQuery, ApiResponse<List<SessionDTO>>>
{
    ...
    public async Task<ApiResponse<List<SessionDTO>>> Handle(GetAllSessionsQuery request, CancellationToken cancellationToken)
    {
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            return new ApiResponse<List<SessionDTO>>(false, "An error occurred while retrieving sessions");
        }
    }
}