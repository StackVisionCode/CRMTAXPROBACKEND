...
/// Handler para obtener CompanyPermissions por TaxUser
public class GetCompanyPermissionsByTaxUserHandler
    : IRequestHandler<GetCompanyPermissionsByTaxUserQuery, ApiResponse<CompanyUserPermissionsDTO>>
{
    private readonly ApplicationDbContext _dbContext;
    private readonly ILogger<GetCompanyPermissionsByTaxUserHandler> _logger;

    public GetCompanyPermissionsByTaxUserHandler(
        ApplicationDbContext dbContext,
        ILogger<GetCompanyPermissionsByTaxUserHandler> logger
    )
    {
    ...
    public async Task<ApiResponse<CompanyUserPermissionsDTO>> Handle(
        GetCompanyPermissionsByTaxUserQuery request,
    ...
    )
    {
        try
        {
            ...
            var customPermissionsQuery = await (
                ...
                where
                    cp.TaxUserId == request.TaxUserId
            ...
            return new ApiResponse<CompanyUserPermissionsDTO>(
                true,
                "CompanyPermissions by TaxUser retrieved successfully",
                userPermissionsDto
            );
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting CompanyPermissions by TaxUser: {TaxUserId}",
                request.TaxUserId
            );
            return new ApiResponse<CompanyUserPermissionsDTO>(
                false,
                "Error retrieving CompanyPermissions by TaxUser",
                null!
            );
        }
    }
