...
/// <summary>
/// Handler para obtener todas las invitaciones del sistema (Developer)
/// </summary>
public class GetAllInvitationsHandler
    : IRequestHandler<GetAllInvitationsQuery, ApiResponse<PagedResult<InvitationDTO>>>
{
    private readonly ApplicationDbContext _dbContext;
    private readonly ILogger<GetAllInvitationsHandler> _logger;

    public GetAllInvitationsHandler(
        ApplicationDbContext dbContext,
        ILogger<GetAllInvitationsHandler> logger
    )
    {
    ...
    public async Task<ApiResponse<PagedResult<InvitationDTO>>> Handle(
        GetAllInvitationsQuery request,
        CancellationToken cancellationToken
    )
    {
        try
        {
            // Query base con joins optimizados para todas las companies
            var baseQuery =
            ...
            // Aplicar filtros
            if (request.CompanyIdFilter.HasValue)
            {
                baseQuery = baseQuery.Where(x => x.CompanyId == request.CompanyIdFilter.Value);
            }
            ...
            _logger.LogDebug(
                "Retrieved {Count} invitations globally (page {Page}/{TotalPages})",
            ...
            return new ApiResponse<PagedResult<InvitationDTO>>(
                true,
                "All invitations retrieved successfully",
                pagedResult
            );
        }
        catch (Exception ex)
