...
public class SendEmailHandler
    : IRequestHandler<SendEmailCommand, EmailDTO>
{
    ...
    public SendEmailHandler(EmailContext ctx, IEmailService svc, IMapper map, ILogger<SendEmailHandler> log)
    {
        _ctx  = ctx;
        _svc  = svc;
        _map  = map;
    ...
    {
        var email = await _ctx.Emails.FirstOrDefaultAsync(
                        e => e.Id == request.EmailId, ct);
        if (email is null)
