...
public class GetAllContactInfoHandler : IRequestHandler<GetAllContactInfoQueries, ApiResponse<List<ReadContactInfoDTO>>>
{
  private readonly ILogger<GetAllContactInfoHandler> _logger;
  private readonly ApplicationDbContext _dbContext;
  private readonly IMapper _mapper;
  public GetAllContactInfoHandler(ILogger<GetAllContactInfoHandler> logger, ApplicationDbContext dbContext, IMapper mapper)
  {
    _logger = logger;
    _dbContext = dbContext;
    _mapper = mapper;
  }
  public async Task<ApiResponse<List<ReadContactInfoDTO>>> Handle(GetAllContactInfoQueries request, CancellationToken cancellationToken)
  {
    try
    {
      var result = await (
      ...
      if (result is null || !result.Any())
      {
        ...
        return new ApiResponse<List<ReadContactInfoDTO>>(false, "No ContactInfo found", null!);
      }

      var contactInfoDtos = _mapper.Map<List<ReadContactInfoDTO>>(result);
      _logger.LogInformation("ContactInfo retrieved successfully: {ContactInfo}", contactInfoDtos);
      return new ApiResponse<List<ReadContactInfoDTO>>(true, "ContactInfo retrieved successfully", contactInfoDtos);
    }
    catch (Exception ex)
    {
